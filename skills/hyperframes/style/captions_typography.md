...
- Copiá el archivo real desde `assets/fonts/` del cliente al proyecto.
...
7. **CTA con verbo + acción concreta**: *"Pedilo en el link"*, no *"Gracias por
   mirar"*.
8. **Nunca inventes un claim.** Si no está en el brief, en la URL o en el
   `CLAUDE.md` del cliente, no va on-screen.
9. **Voseo** si la marca es rioplatense y el `CLAUDE.md` no dice otra cosa.
