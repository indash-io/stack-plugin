...
- **Modelo de imagen**: [nano-banana / gpt-image] — [razón en una línea según `instructions/07_generation.md`: nano-banana si manda producto/foto/logo, gpt-image si manda texto/infografía]
...
**Imágenes de referencia que necesito** (matriz de ángulos por slide):
- Slide 1 ([función]): vista [frontal / espalda / lateral / interior / macro de detalle X] → [✅ ya la tengo / ❌ necesito que me la mandes]
- Slide 2 ([función]): vista [...] → [✅ / ❌]
- ...
- Slide N ([función]): vista [...] → [✅ / ❌]

**[Si hay ❌ → cerrar con esta línea explícita]**:
> Para los slides marcados con ❌ necesito que me mandes esas imágenes antes de generar (sin la referencia, nano banana inventa esos ángulos y salen mal). ¿Las tenés? Si solo tenés la frontal, lo replanteamos: o cambio esos slides a vistas que sí tengamos, o reduzco el carrusel.
...
### Caso H: faltan imágenes de referencia para algún ángulo del producto que el carrusel propone mostrar
→ **No avances a Concept ni Prompt Generation.** El producto se va a inventar en esos slides y va a salir mal (ver `examples/bad/ai_generico.md` MALO #10).

Tenés tres caminos válidos:

**H.1** — El user puede mandar las imágenes faltantes:
> Listo, espero esas imágenes y avanzo. Mientras tanto avanzo con los slides que ya tengo cubiertos.

**H.2** — El user no las tiene pero quiere igual:
→ Reformulá el concept para que **todos** los slides usen vistas derivables de la frontal (3/4 frontal, macro de un detalle frontal, hero shot frontal en distintas escenas, frontal con persona). Decile qué cambiaste:

> No tenés imagen de espalda → cambio el slide 5 (CTA) a un plano frontal de la modelo con el chaleco, conservando la composición invertida con la pill abajo. El detalle de tachas del cuello posterior queda fuera del carrusel — no podemos mostrar lo que no tenemos referencia. Avanzo así.

**H.3** — El user no puede sumar imágenes y no quiere reducir slides:
→ Sé honesto. Decile que el producto va a derivar y la calidad cae:

> Si avanzo sin esas referencias, los slides 4 y 5 van a tener detalles inventados del producto. Te recomiendo: o sumás las imágenes (10 minutos en el celular), o quitamos esos slides. ¿Qué preferís?
