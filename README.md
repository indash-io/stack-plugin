...
| Skill | Qué hace |
|---|---|
| **new-client** | El **onboarding entero de tu marca en Indash, conversando**: el mismo de la app y al mismo lugar. Respondés de a una cosa, o le pasás una carpeta con todo lo que tengas (PDFs, capturas, audios) y te propone a dónde va cada archivo antes de subir nada. Guarda tu material tal cual, sin resumirlo ni completarlo por su cuenta, conecta tu tienda y cierra con el inventario: qué recibimos, qué falta y si podemos arrancar. Después corre el **análisis** (cómo escribís, qué te preguntan, frases textuales, avatar, sistema visual) y te lo muestra para que confirmes qué se usa: nada derivado entra a un brief sin tu OK. Si vas a producir desde Claude Code, al final arma la carpeta local con el `CLAUDE.md` de contexto escrito desde Indash. Es lo que después lee `content-brief`. No consume créditos. Se dispara con *"nuevo cliente"*, *"quiero cargar mi marca en Indash"* o *"el cliente me mandó todo esto, cargalo"*. |
...
1. **Onboardear la marca** (una vez) → *"Nuevo cliente: Acme Foods"* o *"Quiero cargar mi marca en Indash"* → dispara `new-client`: ubica tu workspace, te pide el material que ya tenés (piezas propias, reseñas sin editar, cómo funciona el producto, límites y objetivo) o levanta una carpeta entera, lo deja en el mismo onboarding de la app, corre el análisis para que confirmes qué se usa y, si querés producir desde acá, arma la carpeta del cliente con su `CLAUDE.md`. Si ya hiciste el onboarding en la app, retoma sin repreguntar. Después trabajás **dentro de esa carpeta** para que el contexto se herede.
...
Las skills de producto necesitan **URL de producto + imagen de referencia** (si armaste la carpeta del cliente con `new-client`, ya los tenés en `assets/products/index.md`). Si falta algo, la skill te lo pide y frena. Antes de generar te hace **una sola pregunta consolidada** con defaults; confirmás o editás, y recién ahí genera. Todo entregable se **guarda** en `exports/<tipo>/` (o `briefs/`) con nombre `<AAAA-MM-DD>_<slug>_v<N>`.
...
skills/new-client/              Onboarding de la marca en Indash (carga, tienda, análisis) + carpeta local opcional
