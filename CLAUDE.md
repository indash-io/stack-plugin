...
- **Onboarding / planificación**: `new-client` (crea la estructura del cliente, baja marca + productos del MCP de Indash, escribe el `CLAUDE.md` de marca), `brand-onboarding` (conduce el onboarding de marca de la app desde Claude: escribe al CKB en Indash, no al disco) y `content-brief` (arma el brief del período y orquesta a las de ejecución). No tienen `style/` de prompts. **`content-brief` es un wrapper fino, a propósito**: la metodología (el `AGENT.md` y las skills `ecom-founder` + `content-brief-builder`) vive en mkt-agents y la sirve `get_brand_context` en vivo, la misma que usa el agente de briefs de la app. No copies esos textos acá: habría dos fuentes y divergirían. La skill solo fija el orden de las tools (`get_brand_context` → armar → `save_brief`), el fallback sin contexto y el handoff. **`brand-onboarding` sigue el mismo patrón**: las preguntas de cada paso y qué acepta cada campo los sirve `get_brand_onboarding` con `include_guide` (el copy del onboarding vive en mkt-agents); la skill fija el orden, la regla de procedencia (a ningún campo entra nada que la persona no haya dicho, pegado, subido o aprobado en la conversación) y la mecánica de la carga masiva. `new-client` y `brand-onboarding` no se pisan: una arma la carpeta local, la otra carga Indash, y ninguna exige a la otra.
...
- El repo tiene **dos manifiestos que describen el mismo plugin**:
  ...
  Conviven sin pisarse: cada cliente lee el suyo y **ignora el del otro**. Es lo que hace que las 12 skills + el conector `indash` se puedan instalar fuera de Claude Code.
