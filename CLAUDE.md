...
skills/<skill>/
  ...
  scripts/              Código ejecutable, solo si la skill lo tiene (hoy: `edicion-ugc`)
```

**Skills con código.** Casi todas las skills son prompt-only. Si una trae
ejecutables, van en `scripts/` y el `SKILL.md` los referencia con
`${CLAUDE_PLUGIN_ROOT}/skills/<skill>/scripts/…` **más un fallback explícito**
(la variable no se sustituye fuera de Claude Code). Regla dura: **nada mutable
adentro de la carpeta del plugin** — venvs, modelos y caches se instalan en
`~/.indash/<skill>/` o `~/.cache/`, porque `~/.claude/plugins/cache/` se pisa
entera en cada auto-update del marketplace. El validador no chequea rutas de
`scripts/` (solo mira refs a `.md` de las carpetas estándar), así que esas las
verificás vos.
...
- **Onboarding / planificación**: `new-client` (el onboarding entero de una marca en Indash: ubica el workspace, carga el material al CKB con las tools de onboarding, ofrece `analyze_brand` y la confirmación rama por rama, y al final, opcional, arma la carpeta local con el `CLAUDE.md` de marca escrito desde Indash) y `content-brief` (arma el brief del período y orquesta a las de ejecución). No tienen `style/` de prompts. **`content-brief` es un wrapper fino, a propósito**: la metodología (el `AGENT.md` y las skills `ecom-founder` + `content-brief-builder`) vive en mkt-agents y la sirve `get_brand_context` en vivo, la misma que usa el agente de briefs de la app. No copies esos textos acá: habría dos fuentes y divergirían. La skill solo fija el orden de las tools (`get_brand_context` → armar → `save_brief`), el fallback sin contexto y el handoff. **`new-client` sigue el mismo patrón**: las preguntas de cada paso y qué acepta cada campo los sirve `get_brand_onboarding` con `include_guide` (el copy del onboarding vive en mkt-agents); la skill fija el orden, la regla de procedencia (a ningún campo entra nada que la persona no haya dicho, pegado, subido o aprobado en la conversación), la mecánica de la carga masiva y el circuito del análisis (derivado, nace `pending`, lo confirma un humano). La carpeta local es un paso opcional al final y su `CLAUDE.md` se escribe desde Indash, nunca desde el sitio: el disco nunca es fuente del onboarding. Hasta la 0.18.0 esto eran dos skills (`new-client` + `brand-onboarding`); se unificaron en la 0.19.0 porque eran dos puertas para el mismo cliente.
...
- **Otras piezas de performance**: `ads` (Meta), `ugc-video-prompts`, `ugc-generator` y `all-videos` (video), `hyperframes` (post-producción: edición y armado de la pieza final con HyperFrames — es *prompt-only*, no usa tools del MCP y renderiza local con Node 22+ y FFmpeg), `edicion-ugc` (post-producción determinística: monta clips de avatar/UGC con FFmpeg + whisper-cpp según reglas medidas contra 21 ediciones manuales; **es la única skill con código ejecutable** — `scripts/editar.py` + `scripts/setup.sh` — y su venv vive **fuera** del plugin, en `~/.indash/edicion-ugc/venv`, porque la cache del plugin se pisa en cada auto-update), `email-marketing-ecomm`. Varias se importaron de skills externas y se alinearon a la convención del stack (gate, persistencia en `exports/<tipo>/`, herencia de marca del cliente). `ugc-generator` es la excepción de persistencia: es el proceso de producción end-to-end de UGC y guarda en su propia estructura por video (`<Cliente>/V<N>-<Producto>/` con `SCRIPTS.md`), no en `exports/videos/`.
...
- El repo tiene **dos manifiestos que describen el mismo plugin**:
  ...
  Conviven sin pisarse: cada cliente lee el suyo y **ignora el del otro**. Es lo que hace que las 11 skills + el conector `indash` se puedan instalar fuera de Claude Code.
